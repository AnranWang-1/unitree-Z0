...
class UniArmL1:
    ...
    def __init__(self, config: UniArmL1RobotConfig):
        ...
        # 设置关节偏好：平衡各关节响应
        # 关节名: shoulder_pan, shoulder_lift, elbow_flex, wrist_flex, wrist_roll
        # 值越大，该关节越容易被IK选中移动
        # 设为0表示该关节几乎不动（被正则化强力约束）
        self.ik_solver.set_joint_preferences({
            "shoulder_pan": 0.5,    # yaw 控制
            "shoulder_lift": 0.3,  # 设为0：几乎不动，正则化最强约束
            "elbow_flex": 0.3,     # 设为0：几乎不动，正则化最强约束
            "wrist_flex": 0.3,     # pitch 控制（已反向）
            "wrist_roll": 0.5,     # roll 控制
        })
        ...
        self.last_sim_q_deg = np.rad2deg(self.q_init_sim_rad)  # 6个元素
        self.ee_pose = self.ik_solver.forward_kinematics(self.last_sim_q_deg[:5])
        self.target_pos_base = self.ee_pose[:3, 3]  # 初始末端位置作为目标位置的初始值
        self.target_ori = self.ee_pose[:3, :3]  # 初始末端姿态作为目标姿态的初始值
