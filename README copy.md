...
Unitree XR Teleoperate 是一个轻量化的 UniArm 机械臂遥操作框架，支持三种控制模式与标准化数据采集，采集数据可无缝对接 [unitree_lerobot](https://github.com/unitreerobotics/unitree_lerobot) 进行模仿学习训练与部署。
...
<div align="center">
   <img
      src="media/readme/real_follwer.png?raw=true"
      alt="Uniarm follower
      title="Uniarm follower"
      style="width: 49.5%;"
    />
...
VR 手柄模式需要安装 xrobotoolkit SDK，请参考 [xrobotoolkit 文档](https://github.com/unitreerobotics/xrobotoolkit) 进行配置。
...
默认配置保存在 `teleop/teleop_config.yaml`，可直接修改，也可通过命令行参数覆盖：
...
```
VR 头戴打开 XRoboToolkit app，输入 PC 端IP，点击连接，勾选控制面板中的 controller、send，按住侧建即可开始遥操作。
...
- [SO-ARM100](https://github.com/TheRobotStudio/SO-ARM100.git) — 开源机械臂硬件设计与组装指南，作为 UniArm 硬件参考
